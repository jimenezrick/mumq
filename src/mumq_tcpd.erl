...
handle_connection(Socket, State) when is_port(Socket) ->
    Conn = mumq_stomp:make_conn(Socket),
    Pid = start_delivery_proc(Socket, mumq_stomp:peername(Conn)),
    State2 = State#state{delivery_proc = Pid},
    handle_connection(Conn, State2);
...
handle_frame(State = #state{conn_state = disconnected}, Conn, Frame = #frame{cmd = connect}) ->
    ...
    case authenticate_client(Login, Passcode) of
        ok ->
            ...
            handle_connection(Conn, State#state{conn_state = connected,
                                                session = Session});
        {error, incorrect_login} ->
